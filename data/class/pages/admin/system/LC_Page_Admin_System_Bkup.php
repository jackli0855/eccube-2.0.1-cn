 ...
 * @version $Id: LC_Page_Admin_System_Bkup.php 16765 2007-11-08 14:03:18Z adachi $
 */
class LC_Page_Admin_System_Bkup extends LC_Page {
    ...
    function lfCheckError($array){
        ...
        if (count($ret) > 0) {
            $objErr->arrErr['bkup_name'] = "接应名重复着。请输入别名。";
        } else {
            $objErr->arrErr = array();
        }
    ...
    function lfRestore($bkup_name){
        ...
        if ($err) {
            ...
            if ($err) {
                $objQuery->commit();
                $this->restore_msg = "恢复结束。";
                $this->restore_err = true;
            }else{
                $objQuery->rollback();
                $this->restore_msg = "恢复失败了。";
