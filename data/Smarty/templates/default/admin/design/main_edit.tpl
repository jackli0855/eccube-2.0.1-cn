					...
					<tr>
						...
						<table width="678" border="0" cellspacing="0" cellpadding="0" summary=" ">
							...
							<tr>
								...
								<td bgcolor="#636469" width="638" class="fs14n"><span class="white"><!--コンテンツタイトル-->页面详细设置</span></td>
						...
						<table width="678" border="0" cellspacing="1" cellpadding="8" summary=" ">
							<tr class="fs12n">
								<td bgcolor="#ffffff" align="left" colspan=2>
									...
									<!--{else}-->
										名称：<input type="text" name="page_name" value="<!--{$arrPageData.page_name|escape}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" style="<!--{if $arrErr.page_name != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}--><!--{/if}-->" size="60" class="box60" /><span class="red"> （最多<!--{$smarty.const.STEXT_LEN}-->字）</span>
									<!--{/if}-->
								</td>
							</tr>
							<tr class="fs12n">
								<td bgcolor="#ffffff" align="left" colspan=2>
									...
									URL：<!--{if $arrPageData.edit_flg == 2}-->
										...
										<!--{else}-->
											<!--{$user_URL}--><input type="text" name="url" value="<!--{$arrPageData.directory|escape}--><!--{$arrPageData.filename|escape}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" style="<!--{if $arrErr.url != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}-->;<!--{/if}--> ime-mode: disabled;" size="40" class="box40" />.php<span class="red"> （最多<!--{$smarty.const.STEXT_LEN}-->字）</span>
										<!--{/if}-->
								</td>
							</tr>
							<tr class="fs12n">
								<td bgcolor="#ffffff" align="left">
									<label for="header"><input type="checkbox" name="header_chk" id="header" <!--{$arrPageData.header_chk}-->>使用通用页眉</label>
								</td>
								<td bgcolor="#ffffff" align="left">
									<label for="footer"><input type="checkbox" name="footer_chk" id="footer" <!--{$arrPageData.footer_chk}-->>使用通用页脚</label>
								</td>
							</tr>
							...
							<tr class="fs12n">
								<td bgcolor="#f2f1ec" align="center" colspan=2>
									...
									<div align="right">
										<input type="button" value="放大" onClick="ChangeSize(this, tpl_data, 50, 13, html_area_row)">
									</div>
									<br/>
								</td>
							</tr>
							<tr class="fs12n">
								<td bgcolor="#f2f1ec" align="center" colspan=2>
									<input type='button' value='注册' name='subm' onclick="fnTargetSelf(); fnFormModeSubmit('form_edit','confirm','','');"  />
									<input type='button' value='预览' name='preview' onclick="doPreview(); "  />
								</td>
							</tr>
						</table>
						<!--▲編集画面　ここまで-->
						...
						<table width="678" border="0" cellspacing="1" cellpadding="5" summary=" ">
							<tr class="fs12n">
								<td bgcolor="#f2f1ec" align="center" colspan=3 ><strong>可编辑页面一览</strong></td>
							</tr>
							...
							<tr class="fs12n" height=20>
								...
								<td align="center" width=78 bgcolor="<!--{if $item.page_id == $page_id}--><!--{$smarty.const.SELECT_RGB}--><!--{else}-->#ffffff<!--{/if}-->">
									<input type="button" value="布局" name="layout<!--{$item.page_id}-->" onclick="location.href='./index.php?page_id=<!--{$item.page_id}-->';"  />
									<input type="hidden" value="<!--{$item.page_id}-->" name="del_id<!--{$item.page_id}-->">
								</td>
								<td align="center" width=78 bgcolor="<!--{if $item.page_id == $page_id}--><!--{$smarty.const.SELECT_RGB}--><!--{else}-->#ffffff<!--{/if}-->">
									<!--{if $item.edit_flg == 1}-->
									<input type="button" value="删除" name="del<!--{$item.page_id}-->" onclick="fnTargetSelf(); fnFormModeSubmit('form_edit','delete','page_id',this.name.substr(3));"  />
									<input type="hidden" value="<!--{$item.page_id}-->" name="del_id<!--{$item.page_id}-->">
									<!--{/if}-->
								</td>
							</tr>
							...
							<tr class="fs12n">
								...
								<input type='button' value='制作新的页面' name='subm' onclick="location.href='http://<!--{$smarty.server.HTTP_HOST}--><!--{$smarty.server.PHP_SELF|escape}-->'">
								</td>
							</tr>
						</table>
						<!--▲一覧　ここまで-->
