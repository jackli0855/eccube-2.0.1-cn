						...
						<table width="706" border="0" cellspacing="0" cellpadding="0" summary=" ">
							...
							<tr>
								...
								<table width="678" border="0" cellspacing="1" cellpadding="8" summary=" ">
									...
									<tr>
										<td bgcolor="#f2f1ec" width="160" class="fs12n">邮件负责人照片<span class="red"> *</span><br />[130×130]</td>
										<!--{assign var=key value="charge_image"}-->
										<td bgcolor="#ffffff" width="547" class="fs12n">
										<!--{if $arrFile[$key].filepath != ""}-->
										<img src="<!--{$arrFile[$key].filepath}-->" alt="邮件负责人照片" width="<!--{$arrFile[$key].width}-->" height="<!--{$arrFile[$key].height}-->"><br/><br />
										<!--{/if}-->
										...
										<input type="button" name="btn" value="上传" onclick="fnModeSubmit('upload_image','image_key','<!--{$key}-->');" />
										</td>
									</tr>
									<tr>
									...
									<tr>
										<td bgcolor="#f2f1ec" width="160" class="fs12n">主商品选择<span class="red"> *</span><br />
										[110×120]</td>
										<td bgcolor="#ffffff" width="547" class="fs12n">
											...
											<!--{/if}-->
											<img src="<!--{$image_path}-->" width="<!--{$smarty.const.SMALL_IMAGE_WIDTH}-->" height="<!--{$smarty.const.SMALL_IMAGE_HEIGHT}-->" alt="商品图片main" />
											<input type="hidden" name="main_product_id" value="<!--{$arrForm.main_product_id}-->"/>
											　<a href="#" onclick="win03('./htmlmail_select.php?name=main_product_id','select','450','300'); return false;" target="_blank">商品选择</a><br />
									...
									<tr>
										<td bgcolor="#f2f1ec" width="160" class="fs12n">附属商品选择<span class="red"> *</span></td>
									...
									<tr class="fs12n">
										<td bgcolor="#f2f1ec" width="160">商品图片（<!--{$subProductNum}-->）</td>
										<td bgcolor="#ffffff" width="547">
											...
											<!--{/if}-->
											<img src="<!--{$image_path}-->" width="<!--{$smarty.const.SMALL_IMAGE_WIDTH}-->" height="<!--{$smarty.const.SMALL_IMAGE_HEIGHT}-->" alt="商品图片<!--{$subProductNum}-->" />　
											<input type="hidden" name="<!--{$sub_product_id}-->" value="<!--{$arrFileName[$smarty.section.cnt.iteration].product_id|escape}-->"/>
											　<a href="#" onclick="win03('./htmlmail_select.php?name=<!--{$sub_product_id}-->' ,'select','450','300'); return false;" target="_blank">商品选择</a>
												<!--{assign var=sub_box value="delete_sub`$smarty.section.cnt.iteration`"}-->
											　<!--{if $arrForm[$sub_product_id]}--><input type="checkbox" name="delete_sub<!--{$smarty.section.cnt.iteration}-->" value="1" <!--{if $arrForm[$sub_box] == '1'}-->checked<!--{/if}-->>商品删除<br /><!--{/if}-->
