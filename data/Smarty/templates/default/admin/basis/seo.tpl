...
<form name="form1" id="form1" method="post" action="<!--{$smarty.post.PHP_SELF|escape}-->" onSubmit="return window.confirm('以此进行注册，可以吗？');">
								...
								<!--{if count($arrPageData) > 0 }-->
									...
									<table width="678" border="0" cellspacing="0" cellpadding="0" summary=" ">
										...
										<tr>
											...
											<td bgcolor="#636469" width="638" class="fs14n"><span class="white" style="float:left"><!--コンテンツタイトル--><!--{$item.page_name}--> <!--{$item.url}--></span><a href="#" id="switch<!--{$item.page_id}-->" style="float:right " onClick="fnDispChange('disp<!--{$item.page_id}-->', 'switch<!--{$item.page_id}-->', 'disp_flg<!--{$item.page_id}-->');"><!--{if $disp_flg[$item.page_id] == ""}--><FONT Color="#FFFF99"> >> 不显示</FONT><!--{else}--><FONT Color="#FFFF99"> << 显示</FONT><!--{/if}--></a></td>
									...
									<div id="disp<!--{$item.page_id}-->" style="display:<!--{$disp_flg[$item.page_id]}-->">
										<table width="678" border="0" cellspacing="1" cellpadding="8" summary=" ">
											<tr>
												<td bgcolor="#f2f1ec" width="160" class="fs12n">MetaTag:Author</td>
												<td bgcolor="#ffffff" width="557" class="fs10n">
												<span class="red12"><!--{$arrErr[$item.page_id].author}--></span>
												<input type="text" name="meta[<!--{$item.page_id}-->][author]" value="<!--{$item.author|escape}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" size="60" class="box60" style='<!--{if $arrErr[$item.page_id].author != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}--><!--{/if}-->' /><span class="red"> （最多<!--{$smarty.const.STEXT_LEN}-->字）</span></td>
											</tr>
											<tr>
												<td bgcolor="#f2f1ec" width="160" class="fs12n">MetaTag:Description</td>
												<td bgcolor="#ffffff" width="557" class="fs10n">
												<span class="red12"><!--{$arrErr[$item.page_id].description}--></span>
												<input type="text" name="meta[<!--{$item.page_id}-->][description]" value="<!--{$item.description|escape}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" size="60" class="box60" style='<!--{if $arrErr[$item.page_id].description != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}--><!--{/if}-->' /><span class="red"> （最多<!--{$smarty.const.STEXT_LEN}-->字）</span></td>
											</tr>
											<tr>
												<td bgcolor="#f2f1ec" width="160" class="fs12n">MetaTag:Keywords</td>
												<td bgcolor="#ffffff" width="557" class="fs10n">
												<span class="red12"><!--{$arrErr[$item.page_id].keyword}--></span>
												<input type="text" name="meta[<!--{$item.page_id}-->][keyword]" value="<!--{$item.keyword|escape}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" size="60" class="box60" style='<!--{if $arrErr[$item.page_id].keyword != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}--><!--{/if}-->' /><span class="red"> （最多<!--{$smarty.const.STEXT_LEN}-->字）</span></td>
											</tr>
										</table>
